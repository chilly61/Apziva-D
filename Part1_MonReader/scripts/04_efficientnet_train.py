#!/usr/bin/env python3
"""
MonReader - 04_EfficientNet模型训练
使用EfficientNetB0提取特征 + Random Forest分类
"""
...
print(f"✅ EfficientNetB0加载完成 ({time.time()-start:.1f}s)，输出维度: 1280")


def extract_efficientnet_features(image_path, target_size=(224, 224)):
    """使用EfficientNetB0提取CNN特征"""
    try:
...
def load_segment_features(segment_dict, label):
    ...
    for idx, (seg_id, images) in enumerate(segment_dict.items()):
        ...
        for img_path in images:
            feat = extract_efficientnet_features(str(img_path))
            if feat is not None:
...
def main():
    ...
    print(f"\n💾 保存EfficientNet特征...")
    np.savez(
        OUTPUT_PATH / "04_efficientnet_features.npz",
        X_train=X_train,
        y_train=y_train,
        X_test=X_test,
        y_test=y_test
    ...
    # 保存结果
    results = {
        "feature_type": "EfficientNetB0 (ImageNet预训练)",
        "feature_dim": 1280,
        "model": "RandomForest",
        "accuracy": float(acc),
        "f1_score": float(f1),
    ...
    with open(OUTPUT_PATH / "04_train_results.json", "w") as f:
        json.dump(results, f, indent=2)
    
    print(f"\n✅ EfficientNet训练完成！Accuracy = {acc:.4f}, F1 = {f1:.4f}")
